...
function App() {
  ...
  const { queue, fetchQueue, addTrack, reorderTrack } =
    useQueue();
  const { currentPlayback, currentlyPlayingId, togglePending, pendingRemoval } = usePlayback();
  const currentTrack = useNowPlaying(queue, currentlyPlayingId, currentPlayback?.item ?? undefined);
  const login = async () => {
